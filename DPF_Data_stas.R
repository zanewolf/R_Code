...
ggplot(Fx_fs_0, aes(x=Frequency, y=mean, group=minPress, colour=minPress))+geom_point()+facet_wrap(~maxPress) +   
  geom_smooth(data=Fx_fs_0[Fx_fs_0$minPress=="0",],aes(x=Frequency, y=mean), method="lm", se=FALSE, color="green")+
  geom_smooth(data=Fx_fs_0[Fx_fs_0$minPress=="0.05",], aes(x=Frequency, y=mean), method="lm", se=FALSE, color="red")+
  geom_smooth(data=Fx_fs_0[Fx_fs_0$minPress=="0.1",], aes(x=Frequency, y=mean), method="lm", se=FALSE, color="blue")+
  geom_smooth(data=Fx_fs_0[Fx_fs_0$minPress=="0.25",], aes(x=Frequency, y=mean), method="lm", se=FALSE, color="purple")+xlab("Frequency (Hz)")+ ylab("Thrust (mN)")+scale_color_manual(values=c("0" = "green", "0.05"= "red", "0.1"= "blue","0.25"="purple" ))+theme(legend.position=c(0.93, 0.9), legend.title = element_blank())
...
# +scale_color_manual(values=c("Black" = "black", "Yellow"= "gold", "Orange"= "orange"))+theme(legend.position=c(0.85, 0.9), legend.title = element_blank())+geom_line()
# +geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.5)


# ggplot(data_Fx_mN, mapping=aes(x=Frequency, y=mean, color=as.factor(StepTime)))+geom_point()+geom_line()+theme_bw()

# ggplot(fs_0, aes(x = Frequency , y = mean, fill=as.factor(StepTime)))+facet_wrap(~freq)+ geom_bar(stat='identity', position='dodge')+ylab("Thrust (mN)")+xlab("Maximum Pressure (kPa)")+ labs(fill="StepTime (ms)")
